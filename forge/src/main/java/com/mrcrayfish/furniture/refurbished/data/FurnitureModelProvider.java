...
public class FurnitureModelProvider extends BlockStateProvider
{
    ...
    private void registerExistingResources()
    {
        ...
        WoodType.values().forEach(type -> {
            helper.trackGenerated(new ResourceLocation(Constants.MOD_ID, "block/" + this.sanitizeName(type.name()) + "_particle"), TEXTURE);
        });

        // Registers coloured particle textures
        Arrays.stream(DyeColor.values()).forEach(type -> {
            helper.trackGenerated(new ResourceLocation(Constants.MOD_ID, "block/" + this.sanitizeName(type.getName()) + "_particle"), TEXTURE);
        });
