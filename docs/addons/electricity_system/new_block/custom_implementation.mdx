...
**Sources** are special in the fact they are ticked before all other block entites, including **modules**. This is achieved with a custom ticker system in the mod. To register your **source** into the system, all you need to do is add the following to your block entity.
...
<Tabs className="unique-tabs">
    <TabItem value="1-20-4" label={<div><img src={require('/static/img/minecraft-icon.webp').default}/> 1.20.4</div>} default>
        ...
        public void setLevel(Level level) {
            ...
            this.registerSourceNodeTicker(level); // From ISourceNode
        }
        ```
    </TabItem>
    <TabItem value="1-20-1" label={<div><img src={require('/static/img/minecraft-icon.webp').default}/> 1.20.1</div>} default>
...
**Modules** are simply registered as any normal block entity using a `BlockEntityTicker`. Your block class should extend vanilla's `BaseEntityBlock` otherwise you will have to write your own helper method to create the `BlockEntityTicker`.
